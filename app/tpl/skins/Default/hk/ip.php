...
<?php
	...
	{
		$derp = mysql_fetch_array(mysql_query("SELECT * FROM users WHERE username = '" . filter($_POST['username']) . "'"), MYSQL_ASSOC);
		$lerp = mysql_query("SELECT * FROM users WHERE ip_last = '" . $derp['ip_last'] . "'");
		
		echo "There are " . mysql_num_rows(mysql_query($lerp)) . " account(s) on this IP. <br /><br />";
		while($ferp = mysql_fetch_array(mysql_query($lerp), MYSQL_ASSOC)) {
		echo "<tr><td>" . $ferp['username'] . "</td><td>" . $ferp['email'] . "</td><td>" . $ferp['ip_last'] . "</td></tr>"; }
	} ?>
